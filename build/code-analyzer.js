 ...
 * Run code analysis
 */
export function runCodeAnalysis(projectPath) {
    const issues = [];
 ...
 */
function extractFunctionDefinitions(content, file) {
    const functions = [];
    // Regular functions
 ...
 */
function extractFunctionCalls(content, file) {
    const calls = [];
 ...
 */
function extractHookRegistrations(content, file) {
    const hooks = [];
 ...
 */
function extractHookRemovals(content, file) {
    const hooks = [];
 ...
 */
function extractClassInstantiations(content) {
    const classes = [];
    // new ClassName
 ...
 */
function checkUndefinedFunctions(calledFunctions, definedFunctions) {
    const issues = [];
    ...
    const undefinedCandidates = new Map();
 ...
 */
function checkUnusedFunctions(definedFunctions, calledFunctions, registeredHooks) {
    const issues = [];
    ...
    for (const func of definedFunctions) {
        // Skip if called
        if (calledNames.has(func.name))
            continue;
        // Skip if registered as hook callback
        if (hookCallbacks.has(func.name))
            continue;
        // Skip entry points
        if (entryPoints.has(func.name))
            continue;
        // Skip magic methods
        if (magicMethods.has(func.name))
            continue;
        // Skip private/protected methods (may be used internally)
        if (func.isMethod && func.visibility !== 'public')
            continue;
 ...
 */
function checkOrphanHooks(registeredHooks, definedFunctions) {
    const issues = [];
    const definedNames = new Set(definedFunctions.map(f => f.name));
    for (const hook of registeredHooks) {
        ...
        if (!definedNames.has(hook.callback)) {
            ...
            if (wpCallbacks.includes(hook.callback))
                continue;
            issues.push({
                file: hook.file,
                line: hook.line,
 ...
 */
function checkOrphanRemovals(removedHooks, registeredHooks) {
    const issues = [];
    ...
    for (const removal of removedHooks) {
        ...
        if (!registered || !registered.has(removal.callback)) {
            ...
            if (coreHooks.some(h => removal.name.includes(h)))
                continue;
            issues.push({
                file: removal.file,
                line: removal.line,
 ...
 */
function checkUnusedClasses(definedClasses, instantiatedClasses) {
    const issues = [];
    ...
    for (const [className, info] of definedClasses) {
        ...
        if (entryPointPatterns.some(p => p.test(className)))
            continue;
 ...
 */
function checkDuplicateFunctions(definedFunctions) {
    const issues = [];
    const seen = new Map();
    for (const func of definedFunctions) {
 ...
 */
function checkHookPriorityConflicts(hooks) {
    const issues = [];
 ...
 */
function checkAcceptedArgsMismatch(hooks, definedFunctions, phpFiles, projectPath) {
    const issues = [];
    // Build a map of function name -> parameter count
    const funcParamCounts = new Map();
    for (const file of phpFiles) {
        const content = readFileSync(file, 'utf-8');
 ...
 */
function checkWrongContextHooks(phpFiles, projectPath) {
    const issues = [];
    const adminHooks = [
