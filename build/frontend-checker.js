 ...
 */
export function runFrontendChecks(projectPath) {
    const issues = [];
    const patterns = {
        classNaming: [],
        colorFormats: [],
 ...
 * Check file naming consistency
 */
function checkFileNaming(projectPath, patterns) {
    const issues = [];
 ...
 */
function checkHtmlInPhp(file, content) {
    const issues = [];
    ...
    while ((match = classPattern.exec(content)) !== null) {
        ...
        for (const cls of classes) {
            if (cls.includes('__') || cls.includes('--'))
                ...
                classStyles.add('camelCase');
            else if (cls.includes('-'))
                classStyles.add('kebab-case');
            else if (cls.includes('_') && !cls.includes('__'))
                classStyles.add('snake_case');
        }
    }
 ...
 * Check CSS patterns
 */
function checkCssPatterns(file, content) {
    const issues = [];
    // !important overuse
    ...
    const colorFormats = [];
    if (hexColors > 0)
        colorFormats.push(`hex(${hexColors})`);
    if (rgbColors > 0)
        colorFormats.push(`rgb(${rgbColors})`);
    if (hslColors > 0)
        colorFormats.push(`hsl(${hslColors})`);
 ...
 */
function checkResponsiveDesign(file, content) {
    const issues = [];
 ...
 */
function checkScssPatterns(file, content) {
    const issues = [];
    // Deep nesting
 ...
 */
function checkJsPatterns(file, content) {
    const issues = [];
    // document.write
...
function detectPatterns(content, patterns) {
    ...
    while ((match = classPattern.exec(content)) !== null) {
        ...
        for (const cls of classes) {
            if (cls.includes('__'))
                patterns.classNaming.push('BEM');
            else if (/^[a-z]+[A-Z]/.test(cls))
                patterns.classNaming.push('camelCase');
            else if (cls.includes('-'))
                patterns.classNaming.push('kebab-case');
        }
    }
}
/**
 * Detect CSS patterns
 */
function detectCssPatterns(content, patterns) {
    if (/#[0-9a-fA-F]{3,6}\b/.test(content))
        patterns.colorFormats.push('hex');
    if (/rgba?\(/i.test(content))
        patterns.colorFormats.push('rgb');
    if (/hsla?\(/i.test(content))
        patterns.colorFormats.push('hsl');
    if (/\d+px/.test(content))
        patterns.units.push('px');
    if (/\d+(?:\.\d+)?rem/.test(content))
        patterns.units.push('rem');
    if (/\d+(?:\.\d+)?em/.test(content))
        patterns.units.push('em');
}
/**
 * Analyze cross-file consistency
 */
function analyzeConsistency(patterns, projectPath) {
    const issues = [];
...
function checkAccessibilityPatterns(file, content) {
    const issues = [];
    let match;
    ...
    while ((match = inputPattern.exec(content)) !== null) {
        ...
        if (/type\s*=\s*["'](?:hidden|submit|button|reset|image)["']/i.test(inputTag))
            continue;
        // Check for matching label or aria-label/aria-labelledby
        if (/aria-label\s*=|aria-labelledby\s*=/i.test(inputTag))
            continue;
 ...
 */
function findPatternLine(lines, pattern) {
    for (let i = 0; i < lines.length; i++) {
...
//# sourceMappingURL=frontend-checker.js.map