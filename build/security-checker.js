 ...
 */
export function runSecurityChecks(projectPath) {
    const issues = [];
...
function checkEvalUsage(file, content, lines) {
...
function checkCreateFunction(file, content, lines) {
...
function checkUnserializeInput(file, content, lines) {
...
function checkExtractInput(file, content, lines) {
...
function checkAbspathGuard(file, content) {
...
function checkRedirectWithoutExit(file, content) {
    const issues = [];
...
function checkDirectFileOps(file, content) {
    const issues = [];
...
function checkAjaxNonce(file, content, lines) {
    if (!/wp_ajax_/.test(content))
        return [];
    if (/check_ajax_referer|wp_verify_nonce/i.test(content))
        return [];
    return [{
...
function checkAjaxDie(file, content) {
    const issues = [];
...
function checkRestPermission(file, content) {
    const issues = [];
...
function checkRestPublic(file, content) {
    const issues = [];
...
function checkCapabilityCheck(file, content, lines) {
    if (!/is_admin\s*\(\)|wp_ajax_/.test(content))
        return [];
    if (/current_user_can\s*\(/i.test(content))
        return [];
    return [{
...
function checkDbPrepare(file, content) {
    const issues = [];
...
function checkDbFormat(file, content) {
    const issues = [];
...
// ─── New v2.0 checks ────────────────────────────────────────────
function checkSqlLikeInjection(file, content) {
    const issues = [];
...
function checkNonceMismatch(file, content) {
    const issues = [];
    const fieldActions = [];
...
function checkFileUploadValidation(file, content) {
    if (!/\$_FILES/i.test(content))
        return [];
    if (/wp_check_filetype|wp_handle_upload|wp_handle_sideload/i.test(content))
        return [];
    return [{
...
function checkUnvalidatedRedirect(file, content) {
    const issues = [];
...
function checkShellExecution(file, content) {
    const issues = [];
...
function checkSsrfRisk(file, content) {
    const issues = [];
...
function checkInsecureRandomness(file, content) {
    const issues = [];
...
function checkDirectSuperglobal(file, content, lines) {
    const issues = [];
...
function checkOutputWithoutEscaping(file, content) {
    const issues = [];
...
function checkDeserializationRisk(file, content) {
    const issues = [];
...
// ─── v3.0 checks ────────────────────────────────────────────────
function checkTimingAttack(file, content) {
    const issues = [];
...
function checkInsecureCookies(file, content) {
    const issues = [];
...
function checkPlaintextPasswords(file, content) {
    const issues = [];
...
function checkCronInfiniteRecursion(file, content) {
    const issues = [];
...
function checkOrphanedCronEvents(file, content) {
    const issues = [];
...
function checkMissingCronCleanup(file, content) {
    const issues = [];
    if (!/wp_schedule_(?:event|single_event)/i.test(content))
        return [];
    if (/wp_clear_scheduled_hook|wp_unschedule_event/i.test(content))
        return [];
...
function checkSessionOutsideWP(file, content) {
    const issues = [];
...
function checkVersionExposure(file, content) {
    const issues = [];
...
function checkUserEnumeration(file, content) {
    const issues = [];
...
function checkInfoDisclosure(file, content) {
    const issues = [];
...
function checkWeakTokenGeneration(file, content) {
    const issues = [];
...
function checkMissingWpUnslash(file, content) {
    const issues = [];
