 ...
 */
export function runSubmissionChecks(projectPath) {
    const issues = [];
...
function checkGplLicense(projectPath) {
    const issues = [];
...
function checkStableTagMatch(projectPath) {
    const issues = [];
...
function checkChangelogVersion(projectPath) {
    const issues = [];
    const readmePath = join(projectPath, 'readme.txt');
    if (!existsSync(readmePath))
        return [];
...
function checkNoTracking(projectPath) {
    const issues = [];
...
function checkDismissibleNotices(projectPath) {
    const issues = [];
    const phpFiles = findPhpFiles(projectPath);
    for (const file of phpFiles) {
        const rel = relativePath(file, projectPath);
...
function checkUninstallCleanup(projectPath) {
...
function checkAbspathGuards(projectPath) {
    const issues = [];
    const phpFiles = findPhpFiles(projectPath);
    for (const file of phpFiles) {
        const rel = relativePath(file, projectPath);
...
function checkPrefixing(projectPath) {
    const issues = [];
...
function checkNoCdnDeps(projectPath) {
    const issues = [];
    const phpFiles = findPhpFiles(projectPath);
    const cdnPatterns = [
        /cdn\.jsdelivr\.net/i,
...
function checkNoWordPressTrademark(projectPath) {
    const issues = [];
    const phpFiles = readdirSync(projectPath).filter(f => f.endsWith('.php'));
    for (const file of phpFiles) {
        const content = readFileSync(join(projectPath, file), 'utf-8');
...
function checkRequiredFiles(projectPath) {
    const issues = [];
...
function checkSvnReadiness(projectPath) {
    const issues = [];
...
//# sourceMappingURL=submission-checker.js.map