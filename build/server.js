...
import { detectProjectType, validatePluginHeaders, validateThemeHeaders, validateReadme, validateTextDomain, validatePotFile, validateStableTagMatch, validateGplLicense, validateUninstallCleanup, } from './validators.js';
...
import { getStagedPhpFiles, checkPhpcsInstalled, checkWpcsInstalled, formatPath, } from './utils.js';
export class WpcsMcpServer {
            ...
            try {
                switch (name) {
                    case 'wpcs_check_staged':
                        return await this.checkStagedFiles(toolArgs.working_dir);
                    case 'wpcs_check_file':
                        return await this.checkFile(toolArgs.file_path, toolArgs.working_dir);
                    case 'wpcs_check_directory':
                        return await this.checkDirectory(toolArgs.directory, toolArgs.working_dir);
                    case 'wpcs_fix_file':
                        return await this.fixFile(toolArgs.file_path, toolArgs.working_dir);
                    case 'wpcs_pre_commit':
                        return await this.preCommitWorkflow(toolArgs.working_dir, toolArgs.auto_stage !== false);
                    case 'wpcs_check_php_compatibility':
                        return await this.checkPhpCompatibility(toolArgs.target, toolArgs.php_version, toolArgs.working_dir);
                    case 'wpcs_quality_check':
                        return this.qualityCheck(toolArgs.path);
                    case 'wpcs_validate_project':
                        return this.validateProject(toolArgs.path, toolArgs.text_domain);
                    case 'wpcs_full_check':
                        return await this.fullCheck(toolArgs.path, toolArgs.php_version, toolArgs.text_domain);
                    case 'wpcs_frontend_check':
                        return this.frontendCheck(toolArgs.path);
                    case 'wpcs_code_analysis':
                        return this.codeAnalysis(toolArgs.path);
                    case 'wpcs_submission_check':
                        return this.submissionCheck(toolArgs.path);
                    case 'wpcs_generate_report':
                        return this.reportGenerate(toolArgs.path, toolArgs.format || 'summary', toolArgs.output_file);
                    case 'wpcs_phpstan_check':
                        return await this.phpstanCheck(toolArgs.path, toolArgs.level || 5);
                    default:
    ...
    async checkStagedFiles(workingDir) {
    ...
    async checkFile(filePath, workingDir) {
        if (!filePath) {
    ...
    async checkDirectory(directory, workingDir) {
        if (!directory) {
    ...
    async fixFile(filePath, workingDir) {
        if (!filePath) {
            return this.errorResult('file_path is required');
        }
    ...
    async preCommitWorkflow(workingDir, autoStage = true) {
        // Check prerequisites
        ...
        const stagedFiles = getStagedPhpFiles(workingDir);
        if (stagedFiles.length === 0) {
            return this.successResult('PRE-COMMIT: PASSED\n\nNo staged PHP files to check. Commit can proceed.');
        }
        const filePaths = stagedFiles.map((f) => f.path);
        // Run fix and check workflow
        const { checkResult, fixedFiles, reStageCommand } = await this.phpcsRunner.fixAndCheck(filePaths, workingDir);
        let message = '';
        // Report fixed files
    ...
    async checkPhpCompatibility(target, phpVersion, workingDir) {
        if (!target) {
    ...
    validateProject(projectPath, textDomain) {
        if (!projectPath) {
            return this.errorResult('path is required');
        }
        ...
        if (stableResult.valid) {
            ...
            hasErrors = true;
            for (const error of stableResult.errors)
                message += `  [ERROR] ${error}\n`;
        }
        for (const warning of stableResult.warnings)
            message += `  [WARN] ${warning}\n`;
        message += '\n';
        // 5. GPL License (v2.0)
        message += `5. LICENSE\n${'-'.repeat(30)}\n`;
        ...
        if (gplResult.valid) {
            ...
            hasErrors = true;
            for (const error of gplResult.errors)
                message += `  [ERROR] ${error}\n`;
        }
        for (const warning of gplResult.warnings)
            message += `  [WARN] ${warning}\n`;
        message += '\n';
        ...
        if (potResult.valid && potResult.warnings.length === 0) {
            ...
            if (!potResult.valid)
                hasErrors = true;
            for (const error of potResult.errors)
                message += `  [ERROR] ${error}\n`;
            for (const warning of potResult.warnings)
                message += `  [WARN] ${warning}\n`;
        }
        message += '\n';
        // Summary
    ...
    async fullCheck(projectPath, phpVersion, textDomain) {
        if (!projectPath) {
            return this.errorResult('path is required');
        }
        const projectType = detectProjectType(projectPath);
    ...
    async runPhpCompatibility(target, phpVersion) {
        ...
        try {
            ...
            try {
                ...
                let errors = 0;
                let warnings = 0;
                for (const data of Object.values(result.files)) {
                    errors += data.errors;
    ...
    formatCheckResult(result, workingDir) {
        if (result.success) {
        ...
        for (const file of result.files) {
            if (shown >= MAX_ISSUES) {
                for (const msg of file.messages)
                    omittedBySource.set(msg.source, (omittedBySource.get(msg.source) ?? 0) + 1);
                continue;
            }
    ...
    reportGenerate(projectPath, format, outputFile) {
        if (!projectPath) {
            return this.errorResult('path is required');
        }
        const projectType = detectProjectType(projectPath);
    ...
    async phpstanCheck(projectPath, level) {
        if (!projectPath) {
            return this.errorResult('path is required');
        }
        // Detect PHPStan binary
        let phpstanPath = null;
        const candidates = [
        ...
        if (!phpstanPath) {
            return this.successResult('PHPStan is not installed. To enable static analysis:\n' +
