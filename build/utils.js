 ...
 */
export function setupEnvironmentPath() {
    const warnings = [];
 ...
 */
export function getStagedPhpFiles(workingDir) {
    try {
 ...
 */
export function checkPhpcsInstalled() {
    try {
 ...
 */
export function checkWpcsInstalled() {
    try {
 ...
 */
export function formatPath(filePath, workingDir) {
 ...
 */
export async function autoInstallWpcs() {
    ...
    try {
        ...
        try {
            execSync('composer global config allow-plugins.dealerdirect/phpcodesniffer-composer-installer true', { encoding: 'utf-8', stdio: 'pipe' });
        }
        catch {
        ...
        execSync('composer global require squizlabs/php_codesniffer wp-coding-standards/wpcs phpcompatibility/phpcompatibility-wp dealerdirect/phpcodesniffer-composer-installer', { encoding: 'utf-8', stdio: 'pipe', timeout: 180000 });
 ...
 */
export function ensureComposerInPath() {
    const homeDir = process.env.HOME || process.env.USERPROFILE || '';
    const composerBin = `${homeDir}/.composer/vendor/bin`;
    if (!process.env.PATH?.includes(composerBin)) {
        process.env.PATH = `${composerBin}:${process.env.PATH}`;
