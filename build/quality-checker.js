 ...
 */
export function runQualityChecks(projectPath) {
    const issues = [];
...
// ─── Hook Checks ────────────────────────────────────────────────
function checkEnqueueHooks(file, content, lines) {
    const issues = [];
...
function checkHookUsage(file, content, lines) {
    const issues = [];
...
function checkDirectOutput(file, content) {
    const issues = [];
...
function checkWordPressPatterns(file, content, lines) {
    const issues = [];
    let match;
...
function checkDeprecatedFunctions(file, content) {
    const issues = [];
    ...
    for (const [funcName, info] of deprecatedMap) {
        ...
        while ((match = regex.exec(content)) !== null) {
            ...
            if (/\/\/|\/\*|\*/.test(lineContent.trim().substring(0, 2)))
                continue;
            issues.push({
...
function checkLargeFiles(projectPath, phpFiles) {
    const issues = [];
